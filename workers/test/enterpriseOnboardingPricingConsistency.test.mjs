 ...
 * (real at the time: ₹499), pro ₹2,999/mo (real: ₹1,499), enterprise
 * ₹25,000/mo (real: ₹4,999), mssp ₹75,000/mo (real: ₹9,999) — a customer
 * comparing this endpoint against an actual checkout would see contradictory
 * prices. Now sourced directly from pricingConfig.js so the two can't drift
 * again. (2026-07-14: Starter's real price was later changed to ₹999/month
 * platform-wide — the guard below now protects against ₹499 as the stale
 * figure instead.) */
...
describe('handleEnterpriseWelcome — pricing matches config/pricingConfig.js', () => {
  it('every paid tier price string comes from the canonical config, not a stale copy', async () => {
    ...
    expect(pricing.starter).not.toContain('₹499/mo');
