...
describe('issueScanToken', () => {
  it('issues a well-formed 5-part token with a 5-minute expiry', async () => {
    const env = { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' };
  ...
  it('answers CORS preflight without touching KV', async () => {
    const res = await issueScanToken(new Request('https://x/api/scan/token', { method: 'OPTIONS' }), { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' });
...
describe('verifyScanToken', () => {
  ...
  it('fails open (availability over strictness) if KV is unavailable at the burn step', async () => {
    const env = { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' };
    const token = await getToken(env);
    const brokenEnv = { SECURITY_HUB_KV: { get: async () => { throw new Error('KV down'); }, put: async () => {} }, JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' };
  ...
  it('accepts a cryptographically valid token whose issuance record is not yet visible in KV (eventual consistency)', async () => {
    const issueEnv = { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' };
    const token = await getToken(issueEnv, '1.2.3.4');
    const verifyEnv = { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' }; // fresh store: put not propagated
  ...
  it('still rejects replay after an eventual-consistency first use (tombstone written by verify itself)', async () => {
    const issueEnv  = { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' };
    const token = await getToken(issueEnv, '1.2.3.4');
    const verifyEnv = { SECURITY_HUB_KV: kvStub(), JWT_SECRET: 'test-jwt-secret-for-scan-token-tests' };
