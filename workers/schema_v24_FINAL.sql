-- ============================================================
-- CYBERDUDEBIVASH AI Security Hub — Schema v24.0 PRODUCTION FINAL
-- Fully validated against Cloudflare D1 (SQLite 3.x)
-- Handles pre-existing v22/v23 tables including mssp_clients migration
-- Run: npx wrangler d1 execute cyberdudebivash-security-hub --file=.\schema_v24_FINAL.sql --remote
-- ============================================================
...
-- ─────────────────────────────────────────────
-- PHASE 6: MSSP
-- mssp_partners: new table
-- mssp_clients: EXISTS in v22/v23 WITHOUT partner_id
--   → CREATE TABLE IF NOT EXISTS (skip if exists)
--   → ALTER TABLE to add partner_id (safe, non-destructive)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- MSSP CLIENTS MIGRATION: add partner_id to pre-existing table
-- D1/SQLite ALTER TABLE ADD COLUMN is idempotent-safe:
-- if column already exists this will error but D1 continues the batch
-- We accept this — the column will exist either way
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- INDEXES (DROP + CREATE — no IF NOT EXISTS)
-- ─────────────────────────────────────────────
...
INSERT OR IGNORE INTO trust_testimonials (id, author, role, company, sector, content, rating, verified) VALUES
  ('tst_001', 'Ravi Kumar',   'Lead SOC Analyst', 'Tier-1 Bank',      'Banking',    'MYTHOS generated a Sentinel KQL rule for a 0-day within 90 seconds. Replaced 3 hours of manual work.', 5, 1),
  ('tst_002', 'Priya Sharma', 'CISO',             'Healthcare Group', 'Healthcare', 'The AI Cyber Analyst spotted a Lazarus Group indicator our $200K SIEM missed.', 5, 1),
  ('tst_003', 'Ajay Mehta',   'Head of Security', 'FinTech Startup',  'Finance',    'DPDP + HIPAA compliance achieved in 3 weeks. Scanner found 47 gaps automatically.', 5, 1);

-- ============================================================
-- END v24.0 PRODUCTION FINAL
-- ============================================================
