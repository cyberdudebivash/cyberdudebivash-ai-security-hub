...
// NOTE: every path below was verified live against production (curl, with a
// real trial API key) on 2026-06-29 — none 404. The previous version pointed
// at endpoints that never existed (intel/ioc-lookup, intel/threat-feed,
// cve/search, scan/asm, ai-red-team/simulate, compliance/gap-analysis,
// mssp/white-label, mssp/workspace), so every "first API call" example shown
// to a brand-new developer 404'd. See workers/src/index.js for live route
// registrations and /api/docs for the canonical catalog.
const USE_CASES = {
  threat_intel:     { label: 'Threat Intelligence',    apis: ['/api/intel/ioc', '/api/intel/cve', '/api/intel/actor'], docs: 'https://cyberdudebivash.in/api-docs.html#threat-intel' },
  vulnerability:    { label: 'Vulnerability Management', apis: ['/api/scan/domain', '/api/intel/cve', '/api/scan/vuln-assessment'], docs: 'https://cyberdudebivash.in/api-docs.html#vulnerability' },
  soc_automation:   { label: 'SOC Automation',         apis: ['/api/soc/cases', '/api/soc/dashboard', '/api/soc/cases/metrics'], docs: 'https://cyberdudebivash.in/api-docs.html#soc' },
  ai_security:      { label: 'AI Security & Governance', apis: ['/api/scan/ai-security', '/api/ai-redteam/probe/prompt-injection', '/api/ai-governance/pdf/generate'], docs: 'https://cyberdudebivash.in/api-docs.html#ai-security' },
  compliance:       { label: 'Compliance & Reporting', apis: ['/api/scan/compliance', '/api/reports/executive', '/api/ciso/metrics'], docs: 'https://cyberdudebivash.in/api-docs.html#compliance' },
  mssp:             { label: 'MSSP / Multi-Tenant',    apis: ['/api/mssp/clients', '/api/mssp/onboarding/tiers', '/api/mssp/overview'], docs: 'https://cyberdudebivash.in/api-docs.html#mssp' },
};

const QUICKSTART_STEPS = {
  threat_intel: [
    { step: 1, title: 'Look up an IOC', code: { curl: `curl -H "X-API-Key: YOUR_KEY" "https://cyberdudebivash.in/api/intel/ioc?value=8.8.8.8"`, js: `const r = await fetch('https://cyberdudebivash.in/api/intel/ioc?value=8.8.8.8', { headers: { 'X-API-Key': 'YOUR_KEY' } }); const data = await r.json();`, python: `import requests\nr = requests.get('https://cyberdudebivash.in/api/intel/ioc', params={'value': '8.8.8.8'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
    { step: 2, title: 'Look up a CVE', code: { curl: `curl -H "X-API-Key: YOUR_KEY" "https://cyberdudebivash.in/api/intel/cve?cve_id=CVE-2024-3400"`, js: `const r = await fetch('https://cyberdudebivash.in/api/intel/cve?cve_id=CVE-2024-3400', { headers: { 'X-API-Key': 'YOUR_KEY' } });`, python: `r = requests.get('https://cyberdudebivash.in/api/intel/cve', params={'cve_id': 'CVE-2024-3400'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
    { step: 3, title: 'Get a threat actor profile (PRO+)', code: { curl: `curl -H "X-API-Key: YOUR_KEY" "https://cyberdudebivash.in/api/intel/actor?actor=APT28"`, js: `const r = await fetch('https://cyberdudebivash.in/api/intel/actor?actor=APT28', { headers: { 'X-API-Key': 'YOUR_KEY' } });`, python: `r = requests.get('https://cyberdudebivash.in/api/intel/actor', params={'actor': 'APT28'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
  ],
  vulnerability: [
    ...
    { step: 2, title: 'Look up a CVE', code: { curl: `curl -H "X-API-Key: YOUR_KEY" "https://cyberdudebivash.in/api/intel/cve?cve_id=CVE-2024-3400"`, js: `const r = await fetch('https://cyberdudebivash.in/api/intel/cve?cve_id=CVE-2024-3400', { headers: { 'X-API-Key': 'YOUR_KEY' } });`, python: `r = requests.get('https://cyberdudebivash.in/api/intel/cve', params={'cve_id': 'CVE-2024-3400'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
    { step: 3, title: 'Run a vulnerability assessment (PRO+)', code: { curl: `curl -X POST -H "X-API-Key: YOUR_KEY" -H "Content-Type: application/json" -d '{"domain":"example.com"}' https://cyberdudebivash.in/api/scan/vuln-assessment`, js: `const r = await fetch('https://cyberdudebivash.in/api/scan/vuln-assessment', { method: 'POST', headers: { 'X-API-Key': 'YOUR_KEY', 'Content-Type': 'application/json' }, body: JSON.stringify({ domain: 'example.com' }) });`, python: `r = requests.post('https://cyberdudebivash.in/api/scan/vuln-assessment', json={'domain': 'example.com'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
  ],
  ai_security: [
    { step: 1, title: 'Run an AI security scan', code: { curl: `curl -X POST -H "X-API-Key: YOUR_KEY" -H "Content-Type: application/json" -d '{"domain":"example.com"}' https://cyberdudebivash.in/api/scan/ai-security`, js: `const r = await fetch('https://cyberdudebivash.in/api/scan/ai-security', { method: 'POST', headers: { 'X-API-Key': 'YOUR_KEY', 'Content-Type': 'application/json' }, body: JSON.stringify({ domain: 'example.com' }) });`, python: `r = requests.post('https://cyberdudebivash.in/api/scan/ai-security', json={'domain': 'example.com'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
    { step: 2, title: 'Probe for prompt injection', code: { curl: `curl -X POST -H "X-API-Key: YOUR_KEY" -H "Content-Type: application/json" -d '{"target":"gpt-4"}' https://cyberdudebivash.in/api/ai-redteam/probe/prompt-injection`, js: `const r = await fetch('https://cyberdudebivash.in/api/ai-redteam/probe/prompt-injection', { method: 'POST', headers: { 'X-API-Key': 'YOUR_KEY', 'Content-Type': 'application/json' }, body: JSON.stringify({ target: 'gpt-4' }) });`, python: `r = requests.post('https://cyberdudebivash.in/api/ai-redteam/probe/prompt-injection', json={'target': 'gpt-4'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
    { step: 3, title: 'Generate an AI governance compliance PDF', code: { curl: `curl -X POST -H "X-API-Key: YOUR_KEY" -H "Content-Type: application/json" -d '{"framework":"NIST_AI_RMF"}' https://cyberdudebivash.in/api/ai-governance/pdf/generate`, js: `const r = await fetch('https://cyberdudebivash.in/api/ai-governance/pdf/generate', { method: 'POST', headers: { 'X-API-Key': 'YOUR_KEY', 'Content-Type': 'application/json' }, body: JSON.stringify({ framework: 'NIST_AI_RMF' }) });`, python: `r = requests.post('https://cyberdudebivash.in/api/ai-governance/pdf/generate', json={'framework': 'NIST_AI_RMF'}, headers={'X-API-Key': 'YOUR_KEY'})` } },
  ],
};
