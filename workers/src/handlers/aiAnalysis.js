...
export async function handleGenerateRules(request, env, authCtx = {}) {
  let body;
  try { body = await request.json(); } catch { return Response.json({ error: 'Invalid JSON' }, { status: 400 }); }

  const { cve_id, threat = '', platform = 'all', title: customTitle } = body;
  const cveLabel  = String(cve_id || threat || 'GENERIC-THREAT').slice(0, 120);
  ...
  const yara = `rule CDB_${safeLabel}_Detection
{
  ...
  // STIX 2.1 bundle — Sigma and YARA are the two IANA-registered pattern_type
  // values this engine emits, so the bundle carries exactly those two
  // indicators (SPL/KQL/EQL are not valid STIX pattern types).
  const stix = JSON.stringify(buildRuleStixBundle({ cveLabel, sigma, yara, today }), null, 2);

  const rules = { sigma, splunk, kql, yara, elastic, stix };
  const output = platform === 'all'
    ? rules
    : { [platform]: rules[platform] || `Rule format '${platform}' not available` };

  // Suggested per-format download filenames (frontend uses these verbatim)
  const exportFilenames = {
    sigma:   `${safeLabel}_sigma_${today}.yml`,
    splunk:  `${safeLabel}_splunk_${today}.spl`,
    kql:     `${safeLabel}_sentinel_${today}.kql`,
    yara:    `${safeLabel}_yara_${today}.yar`,
    elastic: `${safeLabel}_elastic_${today}.json`,
    stix:    `${safeLabel}_stix21_bundle_${today}.json`,
    all:     `${safeLabel}_all_platforms_${today}.txt`,
  };

  // Persist to rule history (authenticated users only — anonymous generations
  // have no account to revisit them from, so they are honestly not saved).
  let saved = false, recordId = null, version = null;
  if (env?.DB && authCtx?.user_id) {
    try {
      const persisted = await saveGeneratedRule(env.DB, authCtx.user_id, cveLabel, platform, output);
      saved    = true;
      recordId = persisted.id;
      version  = persisted.version;
    } catch (e) {
      console.error('[RULES] history persist failed:', e?.message);
    }
  }
  ...
  return Response.json({
    ...
    platform,
    rules:     output,
    export_filenames: exportFilenames,
    saved,
    record_id: recordId,
    version,
    history:   authCtx?.user_id ? '/api/ai/rules/history' : 'Sign in to keep a rule generation history',
    generated: today,
    author:    'CYBERDUDEBIVASH MYTHOS AI',
    timestamp: new Date().toISOString(),
  });
...
// ─── STIX 2.1 bundle builder for generated detection rules ──────────────────
function buildRuleStixBundle({ cveLabel, sigma, yara, today }) {
  const now = new Date().toISOString();
  const identityId = 'identity--c9b9d6f1-1e0a-4a6b-9d3e-cdb000000001'; // stable producer identity
  const isCve = /^CVE-\d{4}-\d{4,}$/i.test(cveLabel);
  const externalRefs = isCve
    ? [{ source_name: 'cve', external_id: cveLabel.toUpperCase(), url: `https://nvd.nist.gov/vuln/detail/${cveLabel.toUpperCase()}` }]
    : [];

  const indicator = (patternType, pattern, name) => ({
    type: 'indicator',
    spec_version: '2.1',
    ...
    name,
    description: `${patternType.toUpperCase()} detection rule for ${cveLabel}, generated by CYBERDUDEBIVASH MYTHOS AI Engine.`,
    pattern,
    pattern_type: patternType,
    valid_from: now,
    labels: ['malicious-activity'],
    ...(externalRefs.length ? { external_references: externalRefs } : {}),
  });
  ...
  return {
    ...
    objects: [
      ...
      indicator('sigma', sigma, `${cveLabel} — Sigma exploitation detection`),
      indicator('yara',  yara,  `${cveLabel} — YARA artifact detection`),
    ],
  };
}

// ─── Rule history persistence (D1) ──────────────────────────────────────────
async function ensureGeneratedRulesTable(db) {
  await db.prepare(`
...
async function saveGeneratedRule(db, userId, cveLabel, platform, rules) {
  await ensureGeneratedRulesTable(db);
  const prior = await db.prepare(
    `SELECT COUNT(*) as ct FROM generated_rules WHERE user_id = ? AND cve_id = ? AND platform = ?`
  ).bind(userId, cveLabel, platform).first();
  const version = (prior?.ct ?? 0) + 1;
  const id = 'rule_' + Date.now().toString(36) + '_' + crypto.randomUUID().slice(0, 8);
  await db.prepare(
    `INSERT INTO generated_rules (id, user_id, cve_id, platform, version, rules_json)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, cveLabel, platform, version, JSON.stringify(rules)).run();
  return { id, version };
}

// ─── GET /api/ai/rules/history — user's rule generation history ─────────────
export async function handleRulesHistory(request, env, authCtx = {}) {
  if (!authCtx?.user_id) {
    return Response.json({ error: 'Authentication required to view rule history' }, { status: 401 });
  }
  ...
  try {
    await ensureGeneratedRulesTable(env.DB);
    const rows = cveId
      ? await env.DB.prepare(
          `SELECT id, cve_id, platform, version, created_at FROM generated_rules
           WHERE user_id = ? AND cve_id = ? ORDER BY created_at DESC LIMIT ?`
        ).bind(authCtx.user_id, cveId, limit).all()
      : await env.DB.prepare(
          `SELECT id, cve_id, platform, version, created_at FROM generated_rules
    ...
    return Response.json({
      ...
      note: history.length === 0
        ? 'No detection rules generated yet — generate one with POST /api/ai/generate-rules'
        : 'Fetch full rule content with GET /api/ai/rules/:id',
    });
  } catch (e) {
    return Response.json({ error: 'Failed to load rule history', detail: e?.message }, { status: 500 });
  }
}

// ─── GET /api/ai/rules/:id — full content of one saved rule generation ──────
export async function handleGetSavedRule(request, env, authCtx = {}, recordId) {
  if (!authCtx?.user_id) {
    return Response.json({ error: 'Authentication required' }, { status: 401 });
  }
  if (!env?.DB) return Response.json({ error: 'Database unavailable' }, { status: 503 });
  if (!recordId) return Response.json({ error: 'Rule record ID required' }, { status: 400 });

  try {
    const row = await env.DB.prepare(
      `SELECT id, cve_id, platform, version, rules_json, created_at
       FROM generated_rules WHERE id = ? AND user_id = ?`
    ).bind(recordId, authCtx.user_id).first();
    ...
    return Response.json({ error: 'Failed to load rule', detail: e?.message }, { status: 500 });
  }
}
