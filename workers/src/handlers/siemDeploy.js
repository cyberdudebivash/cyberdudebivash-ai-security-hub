...
export const INTEGRATION_PLATFORMS = {
  ...
  // P8.0-004: native SOAR/SIEM connectors
  qradar: {
    label:       'IBM QRadar',
    icon:        '🔹',
    description: 'IBM QRadar SIEM custom event collector / offense ingestion',
    rule_format: 'leef',
    docs_url:    'https://www.ibm.com/docs/en/qsip',
    field_label: 'QRadar custom event collector / REST API URL',
  },
  google_secops: {
    label:       'Google SecOps',
    icon:        '🟩',
    description: 'Google Security Operations (Chronicle) UDM event ingestion',
    rule_format: 'udm',
    docs_url:    'https://cloud.google.com/chronicle/docs',
    field_label: 'Chronicle UDM ingestion endpoint URL',
  },
  cortex_xsoar: {
    label:       'Cortex XSOAR',
    icon:        '🔺',
    description: 'Palo Alto Cortex XSOAR automated incident creation',
    rule_format: 'json',
    docs_url:    'https://xsoar.pan.dev/docs/about',
    field_label: 'Cortex XSOAR /incident REST API URL',
  },
};
...
function buildPayload(platform, rule, meta = {}) {
  ...
  switch (platform) {
    ...
    case 'qradar':
      // LEEF-equivalent JSON shape for QRadar's custom event collector / REST ingestion app.
      return {
        devTime:       ts,
        ...
        version:       '1.0',
        eventId:       ruleId,
        name:          `MYTHOS Detection: ${meta.cve_id || 'Custom Rule'}`,
        severity:      mapSeverityToQRadar(meta.severity),
        cve:           meta.cve_id || '',
        cvss:          meta.cvss || '',
        sig:           rule.sigma || rule.raw || '',
        src:           'cyberdudebivash-ai-security-hub',
      };

    case 'google_secops':
      // Chronicle Unified Data Model (UDM) event shape.
      return {
        events: [{
          metadata: {
            event_timestamp:     ts,
            event_type:          'GENERIC_EVENT',
            product_name:        'CYBERDUDEBIVASH MYTHOS',
            vendor_name:         'CyberDudeBivash',
            product_event_type:  meta.cve_id ? 'CVE_DETECTION' : 'DETECTION_RULE',
          },
          security_result: [{
            summary:   `MYTHOS Detection: ${meta.cve_id || 'Custom Rule'}`,
            severity:  (meta.severity || 'HIGH').toUpperCase(),
            rule_id:   ruleId,
            rule_name: meta.cve_id || 'CDB Detection Rule',
          }],
          additional: {
            cve_id: meta.cve_id || '',
            cvss:   meta.cvss || '',
            rule:   rule.sigma || rule.raw || '',
          },
        }],
      };

    case 'cortex_xsoar':
      // XSOAR incident-creation payload (POST /incident).
      return {
        name:     `MYTHOS Detection: ${meta.cve_id || 'Custom Rule'}`,
        type:     'Unclassified',
        severity: mapSeverityToXSOAR(meta.severity),
        labels: [
          { type: 'cve_id', value: meta.cve_id || '' },
          { type: 'source',  value: 'CYBERDUDEBIVASH_MYTHOS' },
        ],
        details:  `Auto-generated by CYBERDUDEBIVASH MYTHOS Engine at ${ts}`,
        rawJSON:  JSON.stringify({ rule_id: ruleId, cve_id: meta.cve_id, cvss: meta.cvss, rule: rule.sigma || rule.raw }),
        occurred: ts,
      };
...
// QRadar uses a 1-10 numeric magnitude/severity scale.
function mapSeverityToQRadar(sev) {
  const m = { CRITICAL: 10, HIGH: 8, MEDIUM: 5, LOW: 3 };
  return m[String(sev || 'HIGH').toUpperCase()] ?? 5;
}

// Cortex XSOAR uses 0=Unknown,1=Low,2=Medium,3=High,4=Critical.
function mapSeverityToXSOAR(sev) {
  const m = { CRITICAL: 4, HIGH: 3, MEDIUM: 2, LOW: 1 };
  return m[String(sev || 'HIGH').toUpperCase()] ?? 2;
}
...
async function deployToEndpoint(platformId, config, payload) {
  ...
  } else if (platformId === 'qradar' && config.auth_token) {
    headers['SEC'] = config.auth_token;
  } else if (platformId === 'cortex_xsoar' && config.auth_token) {
    headers['Authorization'] = config.auth_token; // XSOAR expects the raw API key, no Bearer prefix
