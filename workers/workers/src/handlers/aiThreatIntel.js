...
export async function handleScanAgent(request, env, authCtx) {
  ...
  if (dataList.some(d => ['all','*','database','sql'].includes(d.toLowerCase()))) { issues.push({ severity:'HIGH', issue:'Broad database access scope', recommendation:'Restrict data access to specific tables/schemas needed for the agent's task.', owasp_ref:'LLM06' }); riskScore+=20; }
