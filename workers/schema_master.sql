...
CREATE TABLE IF NOT EXISTS mssp_customers (
  ...
  partner_id       TEXT  -- P0 #1: owning partner for per-tenant isolation (schema_v41)
);
...
-- EOP v1.0 tables (applied via schema_migration_eop_a/b/c.sql)
CREATE TABLE IF NOT EXISTS incidents (
  id                TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(8)))),
  title             TEXT NOT NULL,
  description       TEXT,
  severity          TEXT NOT NULL DEFAULT 'minor' CHECK(severity IN ('critical','major','minor','maintenance')),
  status            TEXT NOT NULL DEFAULT 'investigating' CHECK(status IN ('open','investigating','identified','monitoring','resolved')),
...
CREATE TABLE IF NOT EXISTS deployments (
  ...
  status          TEXT NOT NULL DEFAULT 'success' CHECK(status IN ('deploying','success','failed','rolled_back')),
  duration_ms     INTEGER,
  test_count      INTEGER,
