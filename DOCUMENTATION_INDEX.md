...
| Enterprise Product Expansion Program (EPEP) | `docs/product-architecture/epep/00-executive-strategy.md` | Product architecture + business design (not yet implemented) for four proposed products — ARaaS, Threat Intel Automation Pack, Dark Web & Brand Monitoring, AI Security Maturity Assessment. Repository-first capability audit per product with file:line evidence; cross-cutting platform gaps (no Razorpay recurring billing, 5-cron-slot ceiling, dead-code webhook dispatch). Per-product detail in `01`–`04` in the same directory. **Recommended next milestone (not yet implemented):** `05-enterprise-shared-services-platform.md` — a shared scheduling/reporting/campaign/dashboard/notification/billing layer for all four products, motivated by 9 duplicate-implementation patterns (14+ parallel executive-dashboard backends, 5 parallel API-key systems, a silently-broken audit-log writer, and more) found already realized in the current codebase |
