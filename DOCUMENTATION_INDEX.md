...
| Enterprise Product Expansion Program (EPEP) | `docs/product-architecture/epep/00-executive-strategy.md` | Product architecture + business design (not yet implemented) for four proposed products — ARaaS, Threat Intel Automation Pack, Dark Web & Brand Monitoring, AI Security Maturity Assessment. Repository-first capability audit per product with file:line evidence; cross-cutting platform gaps (no Razorpay recurring billing, 5-cron-slot ceiling, dead-code webhook dispatch). Per-product detail in `01`–`04` in the same directory |
