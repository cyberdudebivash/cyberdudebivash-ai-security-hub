...
**Companion documents:** [`01-ai-agent-red-team-as-a-service.md`](01-ai-agent-red-team-as-a-service.md) · [`02-threat-intel-automation-pack.md`](02-threat-intel-automation-pack.md) · [`03-darkweb-brand-monitoring.md`](03-darkweb-brand-monitoring.md) · [`04-ai-security-maturity-assessment.md`](04-ai-security-maturity-assessment.md) · [`05-enterprise-shared-services-platform.md`](05-enterprise-shared-services-platform.md) · **[`06-platform-authority-consolidation-program.md`](06-platform-authority-consolidation-program.md) — now the recommended first engineering milestone, ahead of ESSP and any product**

> **Program update (2026-07-15, second update):** after ESSP was proposed, a further review asked for a Platform Authority Consolidation Program (PACP) first — for each of 10 core platform concerns, name the one current production authority, classify every other implementation, and produce a migration plan, so ESSP builds its shared layer on top of declared authorities rather than on whichever duplicate happens to still be running. §1 and §5 are updated again accordingly. Two concrete, separately-scoped fixes were also shipped this same session, independent of the design work: the SSO audit-log silent-failure bug (dedicated PR) and the CAP-NOTIF-003 webhook-retirement PR (opened, reviewed, in CI) — see §1's GO/NO-GO line for current status of both.
>
> **Prior update (2026-07-15, first update):** a follow-up review proposed the Enterprise Shared Services Platform (ESSP) — a shared layer for scheduling, reports, campaigns, dashboards, alerts, billing, and artifacts that all four products would otherwise each reimplement independently. A fresh repository audit for that review found this failure mode **already realized nine separate times** in the existing codebase (14+ duplicate executive-dashboard backends, 5 duplicate API-key systems, 4 duplicate audit-logging mechanisms including one silently broken, and more — full inventory in the ESSP document §4). The original per-product analysis in §2–§4 and in documents `01`–`04` is unchanged and still current.
...
**Recommended sequencing (updated 2026-07-15 — PACP now precedes ESSP, which precedes all four products):**

| Order | Program/Product | Why |
|---|---|---|
| **0** | **Platform Authority Consolidation Program (PACP)** | **New, recommended first of all.** For 10 core platform concerns (executive dashboards, billing, subscriptions, API keys, campaigns, reports, audit logging, AI orchestration, notifications, usage metering), names one production authority, classifies every other implementation (Derived/Legacy/Dormant/Candidate for retirement), and produces a migration plan — see `06-platform-authority-consolidation-program.md`. Exists so ESSP's shared engines get built on top of declared authorities, not on whichever duplicate happens to still be running |
| **0.5** | **Enterprise Shared Services Platform (ESSP)** | A repository audit for this specific question (see `05-enterprise-shared-services-platform.md`) found the "four products, four duplicate implementations" failure mode already realized nine times over in the existing platform — most urgently, 14+ independent executive-dashboard backends producing inconsistent MRR/risk numbers, and (until fixed this session) a silently-broken audit-logging writer. Building the shared layer, on top of PACP's declared authorities, is assessed as lower total risk than proceeding directly to any product's backend phase |
...
**GO / Conditional GO / NO-GO:** PACP — Conditional GO, starting with Wave 1/2 (its own document's §12: audit-logging and AI-orchestration cleanup, both confirmed zero-risk). ESSP — Conditional GO, sequenced after PACP's authorities are declared. Product 4 — Conditional GO. Product 2 — Conditional GO. Product 1 — Conditional GO for Stage A only; **NO-GO on Stage B pending dedicated legal/security review**. Product 3 — Conditional GO, contingent on resolving the "dark web" naming/positioning question (§2.4 of its companion doc) and adopting the interim pre-paid billing model in §5.1 below. **All four products' own Phase 2 (Backend) should not start until ESSP's Wave 5 (Billing Adapter) and relevant per-product prerequisite waves are in place**, per the ESSP document's wave plan.

**Shipped this session, independent of the design work above (both dedicated, standalone PRs, not bundled into any design document):**
- **SSO audit-log silent-failure fix** — `enterpriseSsoHandler.js`'s audit write referenced a non-existent column and had failed silently on every SSO login. Fixed with a full regression test proving the write lands and is visible through the existing admin audit view; PR open, CI in progress.
- **CAP-NOTIF-003 webhook retirement** — the already-investigated `developerPortal.js` webhook-subsystem retirement branch was merged with current `main`, independently re-reviewed diff-by-diff, full suite re-verified green, and its PR (already open) picked up for review; merge pending CI + final checks.
...
|---|---|---|---|---|---|
| **-1. Platform Authority Consolidation Program** | 🔴 No single authority in 7 of 10 domains today | 🟡 Bounded into 9 waves, several zero-risk | 🟢 Net positive (surfaces the SSO fix, an MSSP key-issuance gap, a fabricated-MRR display) | 🟢 High (unblocks ESSP's shared-engine design) | **First of all — before ESSP** |
| **0. Enterprise Shared Services Platform** | 🔴 Fragmented (9 duplicate-implementation patterns found) | 🟡 Substantial, bounded into 14 waves | 🟢 Net positive (fixes a real silent audit-log failure + a real tenant-isolation gap) | 🟢 High (unblocks recurring billing for all 4 products) | **After PACP, before any product's Phase 2** |
...
1. Approve (or amend) the sequencing in §1/§6 — PACP, then ESSP, then the four products in the stated order.
2. Approve (or amend) PACP's migration sequencing (`06-platform-authority-consolidation-program.md` §12) — Waves 1-2 (audit logging, AI orchestration) are confirmed zero-risk and could start immediately.
3. Two business decisions PACP surfaced that this program does not resolve unilaterally: (a) which number is authoritative for the daily-scan limit — the publicly advertised `pricingConfig.js.daily_scans` or the actually-enforced `TIER_LIMITS.daily_limit` (§11 of the PACP document); (b) whether `multi-agent-os/` (confirmed never deployed) should be deleted outright or someone intends to resume that project.
4. Approve (or amend) ESSP's wave plan (`05-enterprise-shared-services-platform.md` §9), sequenced after PACP.
5. Decide the cross-cutting platform fixes in §3.1/§3.3 (recurring billing, webhook dispatch) — recommend scoping these as their own CAP-numbered production initiatives, following this repo's existing one-problem-per-PR discipline, rather than bundling them into any single product's build. ESSP's wave plan carries these as Waves 5/8/14.
6. For Product 1 specifically: decide whether Stage B (live attack execution) should proceed to a dedicated security/legal review at all, independent of approving Stage A.
7. For Product 3 specifically: decide the "dark web" naming/positioning question and whether to procure a real breach-data source (HIBP key) or a third-party dark-web-intelligence feed.
8. Decide the recurring-billing mechanism (Razorpay Subscriptions API vs. UPI Autopay/eMandate vs. other) — flagged as a business decision in ESSP §12, not made in this program.
9. On approval, PACP's Wave 1 (`06-platform-authority-consolidation-program.md` §12) is ready to begin under this repo's standard one-architectural-concern-per-PR, CI-gated workflow; ESSP and each product's own roadmap remain ready to resume once their respective prerequisites are in place.
10. Status of the two standalone fixes shipped this session: the SSO audit-log PR and the CAP-NOTIF-003 retirement PR are both being actively watched through CI and review — no action needed from you unless a check fails or a review comment needs a decision.
