...
| EH-03 | **Systemic authz smell:** `authCtx.authenticated` is `true` for the anonymous IP-fallback tier, so all ~146 `if (!authCtx.authenticated)` gates fail to require login. Most are read paths that return empty for a null user (low impact); write/state paths (MFA) were exploitable. `/api/keys` etc. use stronger checks and correctly 401. | S2 | 146 call sites; MFA confirmed exploitable, now fixed | **OPEN (documented, not repo-wide-rewritten this pass):** root-cause remediation = introduce an explicit `isRealUser(authCtx)` (JWT/api-key/admin, non-null user_id) helper and migrate state-changing gates to it; audit each of the 146 sites. Deliberately NOT a blind flag-flip (would break intentional free-tier flows). |
