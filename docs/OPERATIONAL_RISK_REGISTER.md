...
| R-19 | **`gitleaks` CI check fails on every PR, unconditionally** — including PRs that touch nothing but a version number in `package-lock.json`. Not caused by any PR's content: the job errors `🛑 missing gitleaks license. Go grab one at gitleaks.io and store it as a GitHub Secret named GITLEAKS_LICENSE` (gitleaks-action v3 made licensing mandatory). Cosmetically this makes every open PR show a red ✕ in the PR list, which reads as "merge conflict" or "broken PR" at a glance even though the actual required check (`CI Gate (All Required Jobs)`) passes independently. | S2 (ops friction / signal quality — real secret-scanning coverage is silently absent, not just cosmetic) | Confirmed 2026-07-24 across 13 consecutive dependency-bump PRs (#284–#296, all merged this pass): `gitleaks` failed identically on all 13, including a 2-line `wrangler` dev-dependency bump with zero surface for a real secret. Job log: `##[error]🛑 missing gitleaks license...` | Get a license at gitleaks.io (free for public repos / small teams — confirm current terms) and store it as repo secret `GITLEAKS_LICENSE`; or if a license isn't wanted, replace with a non-licensed secret scanner (e.g. `trufflehog` or GitHub's own secret scanning) rather than leaving a permanently-red, ignored check — a check everyone learns to ignore stops catching the one time it should. | Platform owner (requires signing up externally; not resolvable from this environment) |
