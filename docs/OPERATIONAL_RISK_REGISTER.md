...
|----|------|-----|----------|--------------------|-------|
| R-06 | **Restore has never been drilled.** Backups now exist but restorability is unproven; the export contains non-idempotent `CREATE TABLE` sections (restore into non-empty DB can error). | S1 | `d1-backup.yml` new this pass; no drill record | **PARTLY RESOLVED (2026-07-02):** shipped `scripts/d1-restore-drill.mjs` — offline drill that restores a dump into a throwaway SQLite DB, runs `PRAGMA integrity_check`, asserts table/row counts + SHA-256. Proven both directions (valid→pass, corrupt/truncated/tampered→fail) in `restoreDrill.test.mjs` (6) and wired into DR runbook §3.0 as the mandatory pre-restore pre-flight. Remaining: run it once against the FIRST real nightly artifact (lands 02:30 UTC; the workflow could not be dispatched early — GitHub App lacks workflow_dispatch scope, 403) → **Platform owner** |
