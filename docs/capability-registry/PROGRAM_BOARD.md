...
| Validator | 0 failures, 0 warnings | `node scripts/registry/validate.mjs`, run 2026-07-12 (after CAP-TIH-013's registry update) |
| Worker test suite | 253 files / 2603 tests passing | `npx vitest run`, run 2026-07-12 — +14 tests this wave, new file `enterpriseIntelligenceDashboard.test.mjs` (CAP-TIH-013: auth-gate + real-D1-driven checks for all 4 previously-untested handlers, a real MITRE ATT&CK correlation check, an honest-empty-campaigns check tied to the radarService.js finding in that entry's notes, plus frontend checks for the new feed section and its new Developer-sidebar nav-item). Baseline going into this wave was 252 files / 2589 tests (CAP-TIH-002). |
