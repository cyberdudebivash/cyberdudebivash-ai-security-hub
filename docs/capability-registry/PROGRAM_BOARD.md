...
| Validator | 0 failures, 0 warnings | `node scripts/registry/validate.mjs`, run 2026-07-12 (after CAP-TIH-007's registry update) |
| Worker test suite | 253 files / 2603 tests passing | `npx vitest run`, run 2026-07-12 — unchanged this wave: CAP-TIH-007 needed zero code/test changes (a stale `navigation.discoverable: "unknown"` field resolved to `true` by direct grep, same treatment as CAP-RBAC-002/CAP-ORG-001). |
...
| Backend / Frontend / Parity | 89.7% / 71.6% / 67% | `PRODUCTION_READINESS_REPORT.md`, regenerated 2026-07-12 — moved again this wave (up from 71.1%/66%): CAP-TIH-013's frontend.status moved `partial`→`exists` (Hidden features/Backend-only unchanged, same reasoning as the CAP-TIH-002 row above — navigation was `unknown` not `false`, and frontend was `partial` not `missing`, so neither counter applied before or after). Sixth consecutive structural gain. `priority` for CAP-TIH-013 stays `P3` per this file's historical-severity convention. |
...
| Gaps by severity | Critical 9 · High 24 · Medium 13 · Low 51 | `PRODUCTION_READINESS_REPORT.md`, regenerated 2026-07-12 — unchanged this wave: `priority` fields are preserved historically rather than recomputed live (see row above), so closing CAP-TIH-013's frontend gap does not move this table. Do not hand-diff against older rows in this table — each was already flagged non-comparable; treat this run as the current baseline |
...
- 12 of 22 remain. Two P3 items left before dropping to P4/P6:
  `CAP-TIH-008` (Global Threat Intel Firehose — `navigation.discoverable:
  false`, "not in top-level nav... reachable only via
  frontend/intel-hub.html", worth checking whether intel-hub.html's own
