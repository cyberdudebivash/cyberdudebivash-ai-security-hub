...
|-----------|-------------|-------|
| Auth: signup / login / session / logout / change-password | **PILOT ONLY** *(updated 2026-07-09, was: NOT READY)* | The 2026-07-08 NOT READY finding (dead-end homepage sign-in modal; no primary-nav path to the real login form) is fixed: CAP-IDN-001 (homepage Sign In entry point), CAP-IDN-002 (signup entry point — a live customer escalation, the homepage's only "Get started free" link went nowhere), and CAP-IDN-003 (MFA second-factor completion, silently mistreated as a successful login) all shipped, each regression-locked. Status is PILOT ONLY, not GA APPROVED, because verification method is `static`/code-level cross-reference plus a local Playwright pass against mocked responses — not yet a `dynamic_browser` pass against live production. See `docs/capability-registry/domains/identity.json` (CAP-IDN-001/002/003) for full evidence. |
...
| Organizations: create / members / RBAC / dashboard / scan history | **PILOT ONLY** *(updated 2026-07-09, was: NOT READY)* | The 2026-07-08 NOT READY finding (zero frontend UI; API-only verification) is fixed: a full org management UI shipped in `frontend/user-dashboard.html` (list/detail views, invite/role-change/remove, settings, danger zone), client-side RBAC cross-checked directly against the backend's own role guards. Status is PILOT ONLY, not GA APPROVED, because `handleOrgScans` (org-wide scan history) remains unwired and verification has not yet reached a `dynamic_browser` pass against live production. See `docs/capability-registry/domains/organizations.json` (CAP-ORG-001) for full evidence. |
