...
| CI-2 | Nightly prod-schema export → CI diff vs `schema_bootstrap.sql` | Would have caught RC-B1 class before ship | **P2 → Code shipped + tested this cycle:** `scripts/d1-schema-diff.mjs` + `.github/workflows/d1-schema-drift.yml` export the live D1 schema nightly (03:40 UTC) via `wrangler d1 execute --json` and diff table existence + column-name sets against `workers/schema_bootstrap.sql` — the exact RC-B1 shape (a column renamed on one side only). 19 unit tests (`workers/test/d1SchemaDiff.test.mjs`), including the RC-B1 scenario itself and a parse-correctness check against the real 227-table `schema_bootstrap.sql` (every table parses with >0 columns, spot-verified against 3 tables by hand). Full suite 136 files / 1,433 tests green. **Not yet live-verified:** first scheduled run against the real production database is still-outstanding evidence, same as CI-1 |
